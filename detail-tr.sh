...
exp=$(grep -E -w "$protocol" "/usr/local/etc/xray/will69.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
uuid=$(cat /usr/local/etc/xray/will69.json | grep -w "$user" | awk {'print $2'} | tail -n 1 | tr -d '"' | sed 's/level://g' | tr -d ',')
