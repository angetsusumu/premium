...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
for user in "${data[@]}"; do
    loguser=$(cat /var/log/xray/access.log | grep -w "$user" | tail -n 500 | cut -d " " -f 3 | sed 's/tcp://g' | cut -d ":" -f 1 | sort -u | wc -l)
    lastlogin=$(cat /var/log/xray/access.log | grep -w "$user" | tail -n 500 | cut -d " " -f 2 | tail -1)
    if [ -f $lastlogin ]; then
        lastlogin='OFFLINE'
    fi
    if [[ -e /etc/usage/vlesswstls/${user} ]]; then
        byt=$(cat /etc/usage/vlesswstls/${user})
        gb=$(convert ${byt})
    else
        gb=""
    fi
